...
-- Tabla de información de la pagina
...
Cerrado'
,'https://cdn-icons-png.flaticon.com/512/4689/4689650.png');
...
CREATE TABLE dts (
    ...
    serialTel_id INT,
    dts_tel DOUBLE,
    INDEX(sex_id,tyDoc_id,city_id,serialTel_id),
    UNIQUE(dts_doc),
    ...
    CONSTRAINT `cityPk_dtsFK_city_id` FOREIGN KEY (city_id) REFERENCES city (city_id) ON DELETE CASCADE ON UPDATE CASCADE,
...
INSERT INTO `srv`(`srv_nam`, `srv_code`, `srv_dsc`, `srv_img`, `srv_prc`, `sta_id`) 
VALUES
...
BEGIN
    ...
    INSERT INTO `dts` VALUES (NULL, nam, ltName, birth, sex, tyDoc, doc, addr, city, serie, tel);
...
-- Ingresar info de la pagina

DELIMITER $$

CREATE PROCEDURE i_infoPag (
    	IN nam VARCHAR(60), 
    	IN inf TEXT,
        IN typ VARCHAR(10),
        IN cod INT,
...
CREATE PROCEDURE v_TypeInfoPag (
    IN pag_typ VARCHAR(10)
)
BEGIN
